...
def img_url(self, filename):
    hash_ = int(time.time())
    return "%s/%s/%s" % ("contact_profile_pics", hash_, filename)
...
class Contact(models.Model):
    ...
    assigned_to = models.ManyToManyField(
        User, blank=True, related_name='contact_assigned_users')
    created_by = models.ForeignKey(
        User, related_name='contact_created_by',
        on_delete=models.SET_NULL, null=True)
    created_on = models.DateTimeField(_("Created on"), auto_now_add=True)
    is_active = models.BooleanField(default=False)
    teams = models.ManyToManyField(Teams, blank=True, related_name='contact_teams')
    ...
    def as_json(self):
        companies=self.company_contacts.all()
        return dict(
            id=self.id,
