...
class TestContactViews(ContactObjectsCreation, TestCase):
    ...
    def test_create_contact(self):
        ...
        response = self.client.post(reverse('contacts:add_contact') + '?view_company={}&address_form='.format(self.company_by_user.id), data)
        ...
        response = self.client.post(reverse('contacts:add_contact') + '?view_company={}'.format(self.company_by_user.id), data,
        ...
        response = self.client.post(reverse('contacts:edit_contact', args=(self.contact.id,)) + '?from_company={}'.format(self.company_by_user.id), data)
        self.assertEqual(302, response.status_code)
        response = self.client.post(reverse('contacts:edit_contact', args=(self.contact.id,)) + '?from_company={}&address_form='.format(self.company_by_user.id), data,
            HTTP_X_REQUESTED_WITH='XMLHttpRequest')
