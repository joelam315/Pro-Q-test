...
class LeadListView(SalesAccessRequiredMixin, LoginRequiredMixin, TemplateView):
    ...
    def get_queryset(self):
        queryset = self.model.objects.all().exclude(status='converted').select_related('created_by'
            ...
            # 'company_name', 'opportunity_amount', 'enquery_type', 'created_from_site',
...
def create_lead(request):
        ...
        if form.is_valid():
            ...
            if request.POST.get('status') == "converted":
                company_object = Company.objects.create(
                    created_by=request.user, name=lead_obj.company_name,
                    email=lead_obj.email, phone=lead_obj.phone,
                    description=request.POST.get('description'),
                    website=request.POST.get('website'),
                )
                company_object.billing_address_line = lead_obj.address_line
                company_object.billing_street = lead_obj.street
                company_object.billing_city = lead_obj.city
                company_object.billing_state = lead_obj.state
                company_object.billing_postcode = lead_obj.postcode
                company_object.billing_country = lead_obj.country
                for tag in lead_obj.tags.all():
                    company_object.tags.add(tag)

                if request.POST.getlist('assigned_to', []):
                    # company_object.assigned_to.add(*request.POST.getlist('assigned_to'))
                    assigned_to_list = request.POST.getlist('assigned_to')
                    current_site = get_current_site(request)
                    recipients = assigned_to_list
                    send_email_to_assigned_user.delay(recipients, lead_obj.id, domain=current_site.domain,
                    ...
                    # for assigned_to_user in assigned_to_list:
                    #     user = get_object_or_404(User, pk=assigned_to_user)
                    #     mail_subject = 'Assigned to company.'
                    #     message = render_to_string(
                    #         'assigned_to/company_assigned.html', {
                    #             'user': user,
                    #             'domain': current_site.domain,
                    #             'protocol': request.scheme,
    ...
    context["companies"] = Company.objects.filter(status="open")
    context["users"] = users
...
def update_lead(request, pk):
    ...
    if request.POST:
        ...
        if request.POST.get('status') == "converted":
            ...
            form.fields['company_name'].required = False
        ...
        if form.is_valid():
            ...
            if request.POST.get('status') == "converted":
                ...
                for comment in lead_obj.leads_comments.all():
                    comment.company = company_object
                    comment.save()
                company_object.save()
            status = request.GET.get('status', None)
            success_url = reverse('leads:list')
            if status:
                success_url = reverse('companies:list')
            return JsonResponse({'error': False, 'success_url': success_url})
        return JsonResponse({'error': True, 'errors': form.errors})
    context = {}
...
def convert_lead(request, pk):
    ...
    if lead_obj.company_name and lead_obj.email:
        ...
        company_object.contacts.add(*contacts_list)
        company_obj = company_object.save()
        current_site = get_current_site(request)
        for assigned_to_user in lead_obj.assigned_to.all().values_list(
                'id', flat=True):
            user = get_object_or_404(User, pk=assigned_to_user)
            mail_subject = 'Assigned to company.'
            message = render_to_string('assigned_to/company_assigned.html', {
                'user': user,
                'domain': current_site.domain,
                'protocol': request.scheme,
                'company': company_object
