...
from companies.models import Company, Email, EmailLog
        ...
        if user:
            if (user.email not in blocked_emails) and (user.email.split('@')[-1] not in blocked_domains):
                ...
                context["url"] = protocol + '://' + domain + \
                    reverse('companies:view_company', args=(company.id,))
                context["user"] = user
                context["company"] = company
                context["created_by"] = created_by
                subject = 'Assigned a company for you.'
                html_content = render_to_string(
                    'assigned_to/company_assigned.html', context=context)
