...
class TestUserMentionsForCompanyComments(CompanyCreateTest, TestCase):

    @override_settings(CELERY_EAGER_PROPAGATES_EXCEPTIONS=True,
                       CELERY_ALWAYS_EAGER=True,
                       BROKER_BACKEND='memory')
    def test_user_mentions_for_company_comment(self):
        ...
        self.comment.company = self.company
        self.comment.save()

        task = send_email_user_mentions.apply((
            self.comment.id, 'companies',),)
        self.assertEqual('SUCCESS', task.state)
...
class TestUserMentionsForQuotationComments(QuotationCreateTest, TestCase):

    @override_settings(CELERY_EAGER_PROPAGATES_EXCEPTIONS=True,
                       CELERY_ALWAYS_EAGER=True,
                       BROKER_BACKEND='memory')
    def test_user_mentions_for_quotation_comments(self):
        self.user_comment = User.objects.create(
            first_name="johnComment", username='johnDoeComment', email='johnDoeComment@example.com', role='ADMIN')
        self.user_comment.set_password('password')
        self.user_comment.save()
        ...
        self.comment.comment = 'content @{}'.format(self.user_comment.username)
        self.comment.quotation = self.quotation
        self.comment.save()

        task = send_email_user_mentions.apply((
            self.comment.id, 'quotations',),)
        self.assertEqual('SUCCESS', task.state)
