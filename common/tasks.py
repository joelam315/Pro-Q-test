...
import six  # Importing six directly
...
def send_email_to_new_user(user_email, created_by, domain='demo.django-crm.io', protocol='http'):
    ...
    if user_obj:
        ...
        subject = 'Welcome to ProQ Control Panel'
...
def send_email_user_mentions(comment_id, called_from, domain='demo.django-crm.io', protocol='http'):
    ...
    if comment:
        ...
        if called_from == 'companies':
            context["url"] = protocol + '://' + domain + \
                reverse('companies:view_company', args=(comment.company.id,))
            subject = 'New comment on Company. '
        ...
        elif called_from == 'quotations':
            context["url"] = protocol + '://' + domain + \
                reverse('quotations:quotation_details', args=(comment.quotation.id,))
            subject = 'New comment on Quotation. '
            ...
            context["url"] = ''
        # subject = 'ProQ Control Panel : comment '
...
def send_email_user_delete(user_email, deleted_by="", domain='demo.django-crm.io', protocol='http'):
    ...
    if user_email:
        ...
        recipients = []
        recipients.append(user_email)
        subject = 'CRM : Your company is Deleted. '
