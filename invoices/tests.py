...
class InvoiceCreateTest(object):
    ...
    def setUp(self):
        ...
        self.invoice.companies.add(self.company.id)
        ...
        self.invoice_1.companies.add(self.company.id)
...
class InvoiceDeleteTestCase(InvoiceCreateTest, TestCase):
    ...
    def test_invoices_delete(self):
        ...
        response = self.client.get(
            reverse('invoices:invoice_delete', args=(self.invoice.id,)) + '?view_company={}'.format(self.company.id,))
        self.assertEqual(response.status_code, 302)
