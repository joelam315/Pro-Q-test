...
class OpportunityDetailView(SalesAccessRequiredMixin, LoginRequiredMixin, DetailView):
    ...
    def get_queryset(self):
        ...
        queryset = queryset.prefetch_related("contacts", "company")
        return queryset
    ...
    def get_context_data(self, **kwargs):
        ...
        if context['object'].company:
            opportunity_company = set([context['object'].company.id])
        else:
            opportunity_company = set()
        if user_assigned_companies.intersection(opportunity_company):
