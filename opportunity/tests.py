...
class OpportunityModel(object):
    ...
    def setUp(self):
        ...
        self.company = Company.objects.create(
            ...
            description="company description", created_by=self.user)
        ...
        self.opportunity = Opportunity.objects.create(
            ...
            closed_on="2016-05-04", company=self.company,
        ...
        self.case = Case.objects.create(
            name="case name", case_type="Problem", status="New", company=self.company,
        ...
        self.attachment = Attachments.objects.create(
            ...
            created_by=self.user, company=self.company, opportunity=self.opportunity)
...
class OpportunityCreateTestCase(OpportunityModel, TestCase):
    ...
    def test_opportunity_create_post(self):
        ...
        data = {'name': "jane opportunity", 'amount': "500", 'stage': "CLOSED WON",
                ...
                'tags': 'tag', 'from_company': self.company.id,
...
class opportunityCreateTestCase(OpportunityModel, TestCase):
    ...
    def test_opportunity_delete(self):
        Opportunity.objects.filter(id=self.company.id).delete()
...
class OpportunityListView(OpportunityModel, TestCase):
    ...
    def test_opportunity_list_queryset(self):
        ...
        data = {
            'name': 'joe', 'stage': 'city',
            'lead_source': 'Call', 'companies': self.company}
        ...
        # get_company = Company.objects.get(name='john')           #  not done
        # print(get_company.name,"wqieoruopwqeiruqrewiou")
        # print(self.company.last())
        self.assertEqual(get_contact.last_name, "doe")
        # self.assertEqual(get_company, self.company.last())      #  not done
        # self.assertEqual(get_opp_val.lead_source,'call')
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'opportunity.html')
...
class TestOpportunityListViewForUser(OpportunityModel, TestCase):
    ...
    def test_queryset_for_user(self):
        ...
        response = self.client.post(reverse('opportunity:list'),
                                    {'company': self.company.id, 'contacts': self.contacts.id})
        self.assertEqual(response.status_code, 200)

        self.company_mp = Company.objects.create(
            name="john doe", email="johnCompany@example.com", phone="911234567892",
...
class AttachmentTestCaseError(OpportunityModel, TestCase):
    ...
    def test_attachment_add(self):
        ...
        response = self.client.get(reverse('opportunity:save') + '?view_company={}'.format(self.company.id))
        self.assertEqual(200, response.status_code)
        ...
        response = self.client.get(reverse('opportunity:opp_edit', args=(self.opportunity_2.id,)) + '?view_company={}'.format(self.company.id))
        self.assertEqual(200, response.status_code)
        ...
        self.client.logout()
        self.client.login(email='joeDoeOpp@example.com', password='password')
        response = self.client.post(reverse('opportunity:opp_remove', args=(self.opportunity.id,)) + '?view_company={}'.format(self.company.id), {
            'pk':self.opportunity.id})
        self.assertEqual(403, response.status_code)

        self.client.logout()
        self.client.login(email='janeOpp@example.com', password='password')
        response = self.client.post(reverse('opportunity:opp_remove', args=(self.opportunity_2.id,)) + '?view_company={}'.format(self.company.id), {
            'pk':self.opportunity_2.id})
        self.assertEqual(302, response.status_code)

        response = self.client.get(reverse('opportunity:contacts') + '?company={}'.format(self.company.id))
        self.assertEqual(200, response.status_code)
