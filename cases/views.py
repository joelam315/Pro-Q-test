...
class CasesListView(AdminAccessRequiredMixin, LoginRequiredMixin, TemplateView):
    ...
    def get_queryset(self):
        ...
        if request_post:
            ...
            if request_post.get('company'):
                queryset = queryset.filter(
                    company_id=request_post.get('company'))
    ...
    def get_context_data(self, **kwargs):
        ...
        context["acc"] = int(self.request.POST.get(
            "company")) if self.request.POST.get("company") else None
        context["case_priority"] = PRIORITY_CHOICE
        context["case_status"] = STATUS_CHOICE

        search = False
        if (
            self.request.POST.get('name') or self.request.POST.get('company') or
...
def create_case(request):
    ...
    kwargs_data = {
        "assigned_to": users, "company": companies, "contacts": contacts}
    ...
    if request.POST:
        ...
        if form.is_valid():
            ...
            if request.POST.get('from_company'):
                ...
                success_url = reverse("companies:view_company", kwargs={
                                      'pk': from_company})
    ...
    if request.GET.get('view_company'):
        context['company'] = get_object_or_404(
            Company, id=request.GET.get('view_company'))
...
class CaseDetailView(AdminAccessRequiredMixin, LoginRequiredMixin, DetailView):
    ...
    def get_queryset(self):
        ...
        return queryset.prefetch_related("contacts", "company")
    ...
    def get_context_data(self, **kwargs):
        ...
        if context['object'].company:
            case_company = set([context['object'].company.id])
        else:
            case_company = set()
        if user_assigned_companies.intersection(case_company):
...
def update_case(request, pk):
    ...
    kwargs_data = {
        "assigned_to": users,
        "company": companies, "contacts": contacts}
...
class RemoveCaseView(AdminAccessRequiredMixin, LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if (
            ...
            self.object.delete()
            if request.GET.get('view_company'):
                company = request.GET.get('view_company')
                return redirect("companies:view_company", pk=company)
...
def select_contact(request):
    contact_company = request.GET.get("company")
    if contact_company:
        company = get_object_or_404(Company, id=contact_company)
