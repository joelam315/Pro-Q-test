...
class CaseCreation(object):
    ...
    def setUp(self):
        ...
        self.company = Company.objects.create(
            name="company name",
            email="company@example.com", phone="12345",
            billing_address_line="",
            billing_street="street",
            billing_city="city",
            billing_postcode="1234",
            billing_country='US',
            website="www.example.com", description="company",
...
class TestCasesListViewForUser(CaseCreation, TestCase):
    ...
    def test_queryset_for_user(self):
        ...
        self.company_mp = Company.objects.create(
                            name="company name", email="johndoe@company.com", phone="911234567892",
    ...
    def test_permissions(self):
        ...
        response = self.client.post(reverse('cases:add_case') + '?view_company={}'.format(self.company.id), {
            'name':'team case',
            'status':'New',
            'priority':'Low',
            'closed_on':'2019-03-14',
        ...
        response = self.client.get(reverse('cases:add_case') + '?view_company={}'.format(self.company.id))
        self.assertEqual(200, response.status_code)


        self.case_without_company = Case.objects.create(
            name="case_without_company", case_type="Problem", status="New",
            priority="Low", description="something",
            created_by=self.user, closed_on="2016-05-04")

        response = self.client.get(reverse('cases:view_case', args=(self.case_without_company.id,)))
        self.assertEqual(200, response.status_code)

        self.company_user = Company.objects.create(
            name="company name",
            email="company@example.com", phone="12345",
            billing_address_line="",
            billing_street="street",
            ...
            website="www.example.com", description="company",
            created_by=self.usermp)
        self.company_user.assigned_to.add(self.usermp.id)
        ...
        response = self.client.get(reverse('cases:edit_case', args=(self.case_user.id,)) + '?view_company={}'.format(self.company.id))
        self.assertEqual(200, response.status_code)
        ...
        response = self.client.get(reverse('cases:remove_case', args=(self.case_user_1.id,)) + '?view_company={}'.format(self.company.id))
        self.assertEqual(403, response.status_code)

        self.client.logout()
        self.client.login(email='johnDoeCase@example.com', password='password')
        response = self.client.get(reverse('cases:remove_case', args=(self.case_user.id,)) + '?view_company={}'.format(self.company.id))
        ...
        self.client.logout()
        self.client.login(email='johnDoeCase@example.com', password='password')
        response = self.client.post(reverse('cases:remove_case', args=(self.case_user.id,)) + '?view_company={}'.format(self.company.id),
        ...
        self.client.logout()
        self.client.login(username='joedoeCase7@user.com', password='password')
        response = self.client.post(reverse('cases:remove_case', args=(self.case.id,)) + '?view_company={}'.format(self.company.id),
            {'case_id': self.case.id})
        self.assertEqual(403, response.status_code)
        ...
        response = self.client.post(reverse('cases:close_case') + '?view_company={}'.format(self.company.id),
            {'case_id': self.case.id})
        self.assertEqual(403, response.status_code)
        data = {
            'caseid': self.case.id,
        ...
        self.client.logout()
        self.client.login(email='johnDoeCase@example.com', password='password')
        response = self.client.get(reverse('cases:select_contacts') + '?company={}'.format(self.company.id))
        self.assertEqual(200, response.status_code)
