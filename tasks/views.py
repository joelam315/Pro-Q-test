...
def task_create(request):
    if request.method == 'GET':
        ...
        #     users = []
        #     companies = Company.objects.filter(created_by=request.user).filter(status="open")
        else:
            users = User.objects.filter(role='ADMIN').order_by('email')
            companies = Company.objects.filter(Q(created_by=request.user) | Q(assigned_to__in=[request.user])).filter(status="open")
        form = TaskForm(request_user=request.user)
        return render(request, 'task_create.html', {'form': form, 'users': users, 'companies':companies, 
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.POST.get('from_company'):
                success_url = reverse('companies:view_company', args=(request.POST.get('from_company'),))
...
def task_detail(request, task_id):
    ...
    if task.company:
        task_companies = set([task.company.id])
    else:
        task_companies = set()
    if user_assigned_companies.intersection(task_companies):
    ...
    if request.method == 'GET':
        ...
        #     task = Task.objects.select_related('company').prefetch_related(
...
def task_edit(request, task_id):
    ...
    if request.method == 'GET':
        ...
        return render(request, 'task_create.html', {'form': form, 'task_obj': task_obj,
            'users': users, 'companies':companies, "teams": Teams.objects.all(),})
...
def task_delete(request, task_id):
    ...
    if request.method == 'GET':
        ...
        if request.GET.get('view_company', None):
